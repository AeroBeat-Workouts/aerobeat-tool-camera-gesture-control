...
func test_camera_gesture_control_manager_creates_controller_and_bumps_repo_version() -> void:
	var manager := AeroCameraGestureControlManager.new()
	assert_eq(AeroCameraGestureControlManager.VERSION, "0.3.0", "Manager version should match the upgraded harness slice")
	assert_eq(
